...
String mqttClientId = System.deviceID();
...
int execute(String json) {
        ...
        for (int i = 0; i < valves.size(); i++) {
                ...
                if (DEBUG || valveController->openValveWithId(valveId)) {
                        ...
                        activatePump();
                        waitWithoutLosingMqttConnection(duration);
                        deactivatePump();
...
void waitWithoutLosingMqttConnection(int seconds) {
...
void loop() {
        ...
        else mqttClient.connect(mqttClientId);
        delay(1000);
}
